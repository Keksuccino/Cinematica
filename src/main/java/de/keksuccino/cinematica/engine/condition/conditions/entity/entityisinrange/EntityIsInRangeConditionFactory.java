package de.keksuccino.cinematica.engine.condition.conditions.entity.entityisinrange;
...
public class EntityIsInRangeConditionFactory extends ConditionFactory {

    public EntityIsInRangeConditionFactory() {
        super("cinematica_condition_entity_is_in_range");
    }
    ...
    public Condition createConditionFromSerializedObject(Condition.SerializedCondition serialized) {
        return new EntityIsInRangeCondition(serialized.identifier, this, serialized.conditionMeta);
    }
    ...
    public void onAddConditionButtonClick(AdvancedButton parentBtn, Screen parentScreen, Cinematic cinematicToAddTheConditionTo) {
        Minecraft.getInstance().setScreen(new EntityRangeConditionScreen(parentScreen, null, (call) -> {
            if (call != null) {
                cinematicToAddTheConditionTo.addCondition(new EntityIsInRangeCondition(null, this, call));
            }
        }));
    }
    ...
    public void onEditConditionButtonClick(AdvancedButton parentBtn, Screen parentScreen, Condition conditionToEdit, Cinematic parentOfCondition) {
        Minecraft.getInstance().setScreen(new EntityRangeConditionScreen(parentScreen, conditionToEdit.conditionMeta, (call) -> {
            if (call != null) {
    ...
    public String getDisplayName() {
        return Locals.localize("cinematica.condition.entity.isinrange");
    }

    @Override
    public List<String> getDescription() {
        return Arrays.asList(StringUtils.splitLines(Locals.localize("cinematica.condition.entity.isinrange.desc"), "%n%"));
    }
