...
public class EnterDimensionConditionFactory extends ConditionFactory {

    public EnterDimensionConditionFactory() {
        super("cinematica_condition_enter_dimension");
    }
    ...
    public Condition createConditionFromSerializedObject(Condition.SerializedCondition serialized) {
        return new EnterDimensionCondition(serialized.identifier, this, serialized.conditionMeta);
    }
    ...
    public void onAddConditionButtonClick(AdvancedButton parentBtn, Screen parentScreen, Cinematic cinematicToAddTheConditionTo) {
        Minecraft.getInstance().setScreen(new DimensionConditionScreen(parentScreen, null, (call) -> {
            if (call != null) {
                cinematicToAddTheConditionTo.addCondition(new EnterDimensionCondition(null, this, call));
            }
        }));
    }
    ...
    public void onEditConditionButtonClick(AdvancedButton parentBtn, Screen parentScreen, Condition conditionToEdit, Cinematic parentOfCondition) {
        Minecraft.getInstance().setScreen(new DimensionConditionScreen(parentScreen, conditionToEdit.conditionMeta, (call) -> {
            if (call != null) {
    ...
    public String getDisplayName() {
        return Locals.localize("cinematica.condition.dimension.enterdimension");
    }

    @Override
    public List<String> getDescription() {
        return Arrays.asList(StringUtils.splitLines(Locals.localize("cinematica.condition.dimension.enterdimension.desc"), "%n%"));
    }
