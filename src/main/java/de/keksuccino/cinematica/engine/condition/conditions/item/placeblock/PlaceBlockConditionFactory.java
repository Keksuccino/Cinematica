...
public class PlaceBlockConditionFactory extends ConditionFactory {
    ...
    public void conditionContextTick() {
        if ((this.placedBlockItem != null) && (Minecraft.getInstance().player != null) && (Minecraft.getInstance().world != null)) {

            PropertiesSection sec = new PropertiesSection("condition-context");
            sec.addEntry("item_type", this.placedBlockItem.getItem().getRegistryName().toString());
            sec.addEntry("item_name", StringUtils.convertFormatCodes(this.placedBlockItem.getDisplayName().getString(), "§", "&"));
