...
public class ChooseFilePopup extends CinematicaTextInputPopup {

    protected AdvancedButton chooseFileBtn;
    private String[] fileTypes;

    public ChooseFilePopup(Consumer<String> callback, String... fileTypes) {
        super(new Color(0, 0, 0, 0), Locals.localize("cinematica.choosefile.enterorchoose"), null, 0, callback);
    ...
    protected void init(Color color, String title, CharacterFilter filter, Consumer<String> callback) {
        ...
        this.chooseFileBtn = new AdvancedButton(0, 0, 100, 20, Locals.localize("cinematica.choosefile.choose"), true, (press) -> {
            PopupHandler.displayPopup(new CinematicaFilePickerPopup(new File("").getAbsoluteFile().getAbsolutePath(), new File("").getAbsoluteFile().getAbsolutePath(), this, true, (call) -> {
                if (call != null) {
