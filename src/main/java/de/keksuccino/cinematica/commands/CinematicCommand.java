    ...
    private static int triggerCinematic(CommandSourceStack stack, String cinematicIdentifier, boolean forceTrigger) {
        try {
            ...
            if (cTemp != null) {
                ...
                stack.sendFailure(new TextComponent(Locals.localize("cinematica.command.cinematic.invalid")));
            }
        } catch (Exception e) {
            stack.sendFailure(new TextComponent(Locals.localize("cinematica.command.cinematic.error")));
            e.printStackTrace();
        }
        return 1;
    }
